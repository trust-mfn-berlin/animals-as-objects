<template>
  <div class="citation-modal" role="dialog" aria-labelledby="citation_label" aria-modal="true" :class="$store.getters.articleTaoType">
    ...
    <p class="f-mono citation" ref="citation" v-if="siteLang == 'en'">{{article.author}}, "{{article.title}}", <em>Animals as Objects?</em>, <span v-if="article.translators.translator_de_en"> trans. {{article.translators.translator_de_en}},</span> 15 November 2021, Last updated {{article.updatedAt | formatDate}}, animalsasobjects.org/{{article.slug}} <span v-if="article.doi">DOI: {{article.doi}}</span></p>
    <p class="f-mono citation" ref="citation" v-else>{{article.author}}, "{{article.title_de}}", <em>Tiere als Objekte?</em>, <span v-if="article.translators.translator_en_de">, Übers. {{article.translators.translator_en_de}},</span> 15. November 2021, letzte Aktualisierung {{article.updatedAt | formatDate}}, animalsasobjects.org/de/{{article.slug}} <span v-if="article.doi">DOI: {{article.doi}}</span></p>
